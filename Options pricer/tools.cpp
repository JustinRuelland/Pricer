...
char* transformation(char chaine[20]){
	//char* p = new char[2];
	char* p;
	p = chaine;
	return p;
...
double* simulation_asset(double S_0, double Sigma, int noeuds, double delta, double riskfree){
	...
	std::normal_distribution<double> simulator(0, 1);

	double spotprice = S_0;
	double* branche = new double[noeuds+1];
	...
	for(int i=1; i<=noeuds; i++){
		W = sqrt(delta/noeuds) * simulator(generator);
		spotprice *= exp( ((r - pow(Sigma, 2)/2) * (delta/noeuds)) + Sigma*W);
		branche[i] = spotprice;
	}
	return branche;
}
...
Eigen::MatrixXd* Asset_Simulation(double S_0, double Sigma, int noeuds, int branche, double delta, double riskfree){
	...
	for(int j=0; j<branche; j++){
		...
		spotprice *= exp( ((r - pow(Sigma, 2)/2) * (delta)) + Sigma*W);
